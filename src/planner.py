...
class Planner:
  def __init__(self, quad, track, Integrator, options = {}):
    ...
    self.fdyn = Integrator(dynamics)      #采用龙格库塔构造四旋翼动力学模型

    # Sizes
    self.NX = dynamics.size1_in(0)
    self.NU = dynamics.size1_in(1)
    self.NW = self.wp.shape[1]           #waypoint的列数为航点数
  ...
  def setup(self):
    ...
    for i in range(self.N):
      ...
      # for j in range(self.NW-1):
      #   g += [xk[6:10]]
      #   lb += [0.9575519102603367, -0.20409495477615966, -0.1990949756094672, -0.042435589765705]
      #   ub += [1.0, 0.20409495477615966, 0.1990949756094672, 0.042435589765705]
