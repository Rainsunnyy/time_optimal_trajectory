...
class Trajectory:
  ...
  def plotTraj(self, fig=None, title='Trajectory', plot_axis='xyz', wp=None, wp_style='rx', **kwargs):
    plot_axis='xyz'
    if 'color' not in kwargs:
      kwargs['color'] = 'b'
  ...
  def plotPos_T(self, fig=None, title='Position_Time', plot_axis=' ', **kwargs):
    if 'color' not in kwargs:
      kwargs['color'] = 'b'
    ...
    if 'x' in plot_axis:
      p += ax.plot(self.t_x, self.p[data[0]], **kwargs)
    ...
    if len(title) > 0:
      ax.set_title(title)  
    ax.set_xlabel('$t$ $[s]$')
    ax.set_ylabel('$p_%c$ $[m]$' % plot_axis[0])
    plt.draw()
    return p
  ...
  def plotOmega(self, fig=None, title='Bodyrate', plot_axis='xyz', **kwargs):
    ...
    ax.legend(lgnd)
    ax.set_xlabel('$t$ $[s]$')
    ax.set_ylabel("$\omega_%c$ $[rad/s]$" % plot_axis[0])
    plt.draw()
    return p
  ...
  def plotOrientation(self, fig=None, title='Orientation', plot_axis='wxyz', **kwargs):
    ...
    ax.legend(lgnd)
    ax.set_xlabel('$t$ $[s]$')
    ax.set_ylabel("$q_%c$ " % plot_axis[0])
    # ax.set_ylabel("$q_%c$ $[unit]$" % plot_axis[0])
    plt.draw()
    return p
